      ...
      <el-table :data="rolesList"
        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row v-for="(level1,l1) in scope.row.children"
                    :key="level1.id"
              ...
              <el-col :span="5">
                <el-tag closable
                        type="primary"
                        @close="deleteRightsById(scope.row,level1.id)">{{level1.authName}}</el-tag>
                <i class="el-icon-caret-right"></i>
              </el-col>
              <!-- 渲染二级和三级权限 -->
              <el-col :span="19">
                <!-- v-for渲染二级权限 -->
                <el-row :class="[l2===0?'':'borderTop','hCenter']"
                        v-for="(level2,l2) in level1.children"
                        :key="level2.id">
                  <el-col :span="6">
                    <el-tag closable
                            type="success"
                            @close="deleteRightsById(scope.row,level2.id)">{{level2.authName}}</el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <!-- 渲染三级权限 -->
                  <el-col :span="18">
